...
expr_without_constants:  T_identifier	{$$ = Construct_AST((char*)yylval,NULL,NULL);}
		| expr T_plus expr				{$$ = Construct_AST("+",$1,$2);}
		| expr T_minus expr				{$$ = Construct_AST("-",$1,$2);}
		| expr T_divide expr			{$$ = Construct_AST("/",$1,$2);}
		| expr T_multiply expr			{$$ = Construct_AST("*",$1,$2);}
		| expr T_mod expr				{$$ = Construct_AST("%",$1,$2);}
		| expr T_LogicalAnd expr		{$$ = Construct_AST("&",$1,$2);}
		| expr T_LogicalOr expr			{$$ = Construct_AST("|",$1,$2);}
		| expr T_less expr				{$$ = Construct_AST("<",$1,$2);}
		| expr T_less_equal expr		{$$ = Construct_AST("<=",$1,$2);}
		| expr T_greater expr			{$$ = Construct_AST(">",$1,$2);}
		| expr T_greater_equal expr		{$$ = Construct_AST(">=",$1,$2);}
		| expr T_equal_equal expr		{$$ = Construct_AST("==",$1,$2);}
		| expr T_not_equal expr			{$$ = Construct_AST("!=",$1,$2);}
		;
