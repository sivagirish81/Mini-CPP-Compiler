...
//%type <NODE> T_identifier T_stringLiteral T_numericConstants
...
%type <NODE> expr_without_constants expr expr_or_empty
...
for_stmt : T_for T_openParenthesis expr_or_empty T_Semicolon expr_or_empty T_Semicolon expr_or_empty T_closedParanthesis block	// {$$ = mknode($3, $6, "For"); printtree($$); printf("\n");}

while_stmt : T_while T_openParenthesis expr T_closedParanthesis block															{$$ = mknode($3, $5, "While"); printtree($$); printf("\n");}

if_stmt : T_if T_openParenthesis expr T_closedParanthesis block elseif_else_empty												{$$ = mknode($3, $5, "If"); printtree($$); printf("\n");}

elseif_else_empty : T_else T_if T_openParenthesis expr T_closedParanthesis block elseif_else_empty								{$$ = mknode($4,$6,"elseif");printtree($$);printf("\n");}
...
Assignment_stmt: 	T_identifier T_AssignmentOperator expr																		{$$ = mknode($1,$3,"=");printtree($$);}
					| T_identifier T_shortHand expr																				{$$ = mknode($1,$3,"SE");printtree($$);}
					| T_type T_identifier T_AssignmentOperator expr_without_constants   {insert_in_st($1, $2, st[top], "j");$$ = mknode($2,$4,"=");printtree($$);printf("/n");}	
					| T_type T_identifier T_AssignmentOperator T_stringLiteral   {insert_in_st($1, $2, st[top], $4);$$ = mknode($2,$4,"=");printtree($$);printf("/n");}
					| T_type T_identifier T_AssignmentOperator T_numericConstants   {insert_in_st($1, $2, st[top], $4);$$ = mknode($2,$4,"=");printtree($$);printf("/n");}
					| T_int T_identifier T_AssignmentOperator expr_without_constants    {insert_in_st($1, $2, st[top], "j");$$ = mknode($2,$4,"=");printtree($$);printf("/n");}
					| T_int T_identifier T_AssignmentOperator T_numericConstants    {insert_in_st($1, $2, st[top], $4);$$ = mknode($2,$4,"=");printtree($$);printf("/n");}
				;
